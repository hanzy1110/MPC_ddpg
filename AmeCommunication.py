...
if sys.platform.startswith('linux'):
   sys.path.append(os.path.abspath(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'lnx_x64')))
else:
   sys.path.append(os.path.abspath(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'win64_vc140')))
...
class AmeSocket(AmeExchanger):
    ...
    def init(self, isserver, server_name, server_port, nb_input, nb_output):
        ...
        ret = binding_amecommunication.sockinit(isserver_int, server_name, server_port, nb_input, nb_output)
        if ret[0] != 0:
...
class AmeSharedmem(AmeExchanger):
    ...
    def init(self, ismaster, name, nb_input, nb_output):
        ...
        if ret[0] != 0:
           raise RuntimeError('unable to initialize shared memory (' + ret[0].__str__() + ')')
        self.shm_id = ret[1]
        self.connected = True
    ...
    def close(self):
        ...
        if ret != 0:
           raise RuntimeError('failed to close shared memory (' + ret[0].__str__() + ')')
        self.shm_id = 0
        self.connected = False
